...
faux_Zj <- function(inp_xvec, inp_gfun, inp_Dvec)
{
  ...
  vec_a <- vec_a[!(is.na(vec_a))]
  #vector b contains x2,x3,...x_k
  vec_b <- inp_xvec[2:k]
  vec_b <- vec_b[!(is.na(vec_b))]
  ...
  #Case 1: when denominator=0, i.e. vec_hPrimea=vec_hPrimeb
  if(identical(all.equal(vec_hPrimea,vec_hPrimeb),TRUE)) {
    out_Zvec[2:k]  <- (vec_a + vec_b)/2  
    return(out_Zvec)
  }
  #Case 2: when denomiantor is not 0 
  else {
